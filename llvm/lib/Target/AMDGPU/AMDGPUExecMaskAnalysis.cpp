...
void AMDGPUExecMaskAnalysis::analyze(MachineFunction &TheMF, LiveIntervals *LIS,
  ...
  do {
    ...
    for (MachineInstr &MI : *MBB) {
      ...
      if (Opcode == AMDGPU::S_CBRANCH_EXECNZ) {
        ...
        CurrentVN = ZeroVN;
      } else if (MI.modifiesRegister(ExecReg, TRI) && Filter(&MI)) {
        bool NeedVN = true;
        if (Opcode == AMDGPU::ENTER_STRICT_WQM ||
          ...
          NeedVN = false;
                   ...
                   (!MI.getOperand(0).isReg() ||
                    MI.getOperand(0).getReg() != ExecReg)) {
          // Ignore COPY operations which implicitly define EXEC
          NeedVN = false;
        } else if (Opcode == OrOpc || Opcode == OrSaveExecOpc) {
          ...
          if (SrcOp->isReg() && hasExecUse(&MI)) {
            ...
            if (Src && Src->getOpcode() == AMDGPU::COPY) {
              if (Src->getOperand(1).isReg() &&
                  Src->getOperand(1).getReg() == ExecReg) {
  ...
  for (auto &E : BlockVN) {
    ...
    if (MI->getOpcode() == OrOpc && (SrcOp = getNonExecUse(MI)) &&
        SrcOp->isReg()) {
      ...
      bool HasMov0Init = InitMI && InitMI->getOpcode() == MovOpc &&
                         InitMI->getOperand(1).isImm() &&
                         InitMI->getOperand(1).getImm() == 0;
      bool HasOrUpdate = UpdateMI && UpdateMI->getOpcode() == OrOpc &&
      ...
      if (UpdateSeg) {
        for (auto &UseMI : MRI->use_nodbg_instructions(SrcReg)) {
          ...
          if (Idx > UpdateSeg->start && Idx < UpdateSeg->end) {
            if (UseMI.getOpcode() == AndN2TermOpc &&
                UseMI.readsRegister(ExecReg, TRI)) {
              HasAndN2Use = true;
              break;
            }
          }
  ...
  std::function<ExecDef(MachineInstr *, bool, bool &)> resolveExecComp =
    ...
    if (Opcode == MovOpc || Opcode == MovTermOpc || Opcode == AMDGPU::COPY) {
      auto &SrcOp = MI->getOperand(1);
      if (!SrcOp.isReg())
        return ExecDef(ED_EXT, MI);
      if (SrcOp.getReg() == ExecReg)
    ...
    if (Opcode == AndOpc || Opcode == AndTermOpc || Opcode == AndN2Opc ||
        Opcode == AndN2TermOpc || Opcode == XorOpc || Opcode == XorTermOpc ||
        Opcode == OrOpc) {
      ...
      if (Opcode == AndOpc || Opcode == AndTermOpc) {
        DefType = ED_AND;
      } else if (Opcode == AndN2Opc || Opcode == AndN2TermOpc) {
        DefType = ED_AND;
        InvertRHS = true;
      } else if (Opcode == XorOpc || Opcode == XorTermOpc) {
        DefType = ED_XOR;
      } else if (Opcode == OrOpc) {
        DefType = ED_OR;
      }
    ...
    if (Opcode == OrSaveExecOpc || Opcode == AndSaveExecOpc ||
        Opcode == AndSaveExecTermOpc) {
      const unsigned ExecVN = VNs.findLeader(InstrVN[MI]);
      if (!Root)
      ...
      ExecDefType DefType = Opcode == OrSaveExecOpc ? ED_OR : ED_AND;
